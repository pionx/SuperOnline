//-------------------------------------carrito
$('#imagen-super').html("<a href='producto'><img id='super-imagen' width='230px' height='165px' class='col-lg-12' src='/Images/mis-imagenes/super-mercados/Jumbo.png'></a>");

var carrito = [];

obtenerCarritoJumboDeSession();
...
function obtenerCarritoJumboDeSession() {
    var postData = { values: "" };

    $.ajax({
        type: "GET",
        url: "/Carrito/obtenerCarritoJumboDeSession",
...
function guardarCarritoJumboEnSession() {
    var postData = { articulos: JSON.stringify(carrito) };

    $.ajax({
        type: "POST",
        url: "/Carrito/guardarCarritoJumboEnSession",
        data: postData,
        success: function (data) {
...
function agregarArticuloAlDivCarrito(articulo) {
    ...
    imgDiv = crearDivImg("/Images/mis-imagenes/Imagenes-productos/" + articulo.nombre + ".PNG"); //~/Images/mis-imagenes/Imagenes-productos/@(articulo.nombre).PNG
    ...
    //$('#carrito-contenido').append(articuloDiv).fadeIn();
